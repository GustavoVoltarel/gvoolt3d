...
- **products.json** — Lista de produtos em formato JSON. Para adicionar um novo produto, adicione um novo objeto com os campos `id`, `name`, `description`, `longDescription`, `price`, `image` e `images`. O campo `description` é a legenda curta exibida na página inicial; `longDescription` é a descrição completa exibida na página de detalhes.
...
2. **Adicione seus produtos** editando o arquivo `products.json`. Use um identificador único em `id`, o nome do produto em `name`, a legenda curta em `description`, a descrição completa em `longDescription`, o preço em `price` (apenas números com ponto como separador decimal), a imagem da página inicial em `image` e as imagens da tela de detalhes em `images`. As imagens podem ser hospedadas no próprio repositório (por exemplo, na pasta `assets/products`) ou em outro serviço de hospedagem.
